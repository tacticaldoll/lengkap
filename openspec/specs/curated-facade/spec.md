...
`lengkap-contract` and SHALL NOT define a second copy or wrapper of any contract
type.
...
- **WHEN** a consumer depends on `lengkap` and imports the evidence-completion
  API
- **THEN** all contract types and functions needed for adjudication are
  available from the facade root
